...
GOSRC=${GOPATH}/src/github.com/nanfengpo
...
do
  ...
  do
    ...
    do
      ...
      rm -f $GOPATH/bin/nanfengpo
      rm -f $GOPATH/bin/init-db
      # Build nanfengpo server and database initializer for RethinkDb and MySQL.
      ~/go/bin/gox -osarch="${plat}/${arc}" \
        -ldflags "-s -w -X main.buildstamp=`git describe --tags`" \
        -tags ${dbtag} -output $GOPATH/bin/nanfengpo ./server > /dev/null
      ~/go/bin/gox -osarch="${plat}/${arc}" \
        -ldflags "-s -w" \
        -tags ${dbtag} -output $GOPATH/bin/init-db ./nanfengpo-db > /dev/null
      ...
      cp ./server/nanfengpo.conf ./releases/tmp
      ...
      cp ./server/static/nanfengpo.js ./releases/tmp/static
      cp ./server/static/drafty.js ./releases/tmp/static
      cp ./nanfengpo-db/data.json ./releases/tmp
      cp ./nanfengpo-db/*.jpg ./releases/tmp
      cp ./nanfengpo-db/credentials.sh ./releases/tmp

      # Build archive. All platforms but Windows use tar for archiving. Windows uses zip.
      if [ "$plat" = "windows" ]; then
        # Copy binaries
        cp $GOPATH/bin/nanfengpo.exe ./releases/tmp
        cp $GOPATH/bin/init-db.exe ./releases/tmp
        cp $GOPATH/bin/keygen.exe ./releases/tmp

        # Remove possibly existing archive.
        rm -f ./releases/${version}/nanfengpo-${dbtag}."${plat}-${arc}".zip
        # Generate a new one
        pushd ./releases/tmp > /dev/null
        zip -q -r ../${version}/nanfengpo-${dbtag}."${plat}-${arc}".zip ./*
        popd > /dev/null
      else
        plat2=$plat
        ...
        # Copy binaries
        cp $GOPATH/bin/nanfengpo ./releases/tmp
        cp $GOPATH/bin/init-db ./releases/tmp
        cp $GOPATH/bin/keygen ./releases/tmp

        # Remove possibly existing archive.
        rm -f ./releases/${version}/nanfengpo-${dbtag}."${plat2}-${arc}".tar.gz
        # Generate a new one
        tar -C ${GOSRC}/chat/releases/tmp -zcf ./releases/${version}/nanfengpo-${dbtag}."${plat2}-${arc}".tar.gz .
      fi
    done
  done
done
...
echo "Building the binary for the demo at api.nanfengpo.co"
...
~/go/bin/gox -osarch=linux/amd64 \
  ...
  -tags rethinkdb -output $GOPATH/bin/nanfengpo ./server > /dev/null
~/go/bin/gox -osarch=linux/amd64 \
  -tags rethinkdb -output $GOPATH/bin/init-db ./nanfengpo-db > /dev/null
